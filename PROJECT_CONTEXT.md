...
│   ├── App.tsx                 # Estado global, theme toggle, colorful mode toggle, mobile drawer, export PNG spinner, modal prerrequisitos, ConfirmDialog, FAB Lista Rápida, e importación perezosa (React.lazy + Suspense) de ColumnMappingDialog.
...
│   │   ├── ColumnMappingDialog.tsx # Modal de mapeo de columnas CSV con Radix UI Select + Motion (`AnimatedSelect`) con focus-visible en el Trigger, z-[200] backdrop y z-[300] menús flotantes.
...
│   │   ├── SubjectSelectionModal.tsx # Modal de "Selección Rápida": listado completo con buscador inteligente multi-campo (materia, profesor, grupo con manejo de prefijo "Grupo", horario formateado y sala), filtrado por materias aprobadas (respetando showAntecedentes), detección de conflictos de horario y badges de estado.
│   │   └── Uploader.tsx        # Carga de CSV y Excel con interceptor para hojas, label HTML nativo con input sr-only y estilos adaptados para mobile, fingerprint → cache/auto-suggest → dialog. Exporta helpers `processCsvFiles` y `processFileAfterMapping`.
...
- **Búsqueda Multi-campo en Selección Rápida (`SubjectSelectionModal.tsx`):** Expansión del filtro de búsqueda para permitir consultar no solo por asignatura y profesor, sino también por **grupo** (compatible con búsqueda literal como "A" o con prefijo "Grupo A"), **horario semanal formateado** (ej. "Lunes", "07:00") y **sala**, con placeholder explicativo actualizado.
- **Optimización de Carga Inicial (Lazy Loading & Bundle Splitting):** Importación perezosa (`React.lazy` + `Suspense`) de `ColumnMappingDialog` en `App.tsx` y separación de `@radix-ui` fuera del chunk `vendor` principal en `vite.config.ts`, reduciendo el bundle inicial del usuario en ~17 kB gzippeados.
- **Corrección de Accesibilidad en Select Trigger (`ColumnMappingDialog.tsx`):** Sustitución de `focus:` por `focus-visible:` en las clases de `Select.Trigger`, solucionando el problema del borde/anillo azul persistente al hacer clic con el mouse.
- **Rediseño Responsive Táctil de Header Móvil (`App.tsx` & `Uploader.tsx`):** Aumento moderado de padding, fuentes e íconos exclusivamente en pantallas móviles (<640px) manteniendo compacidad en el eje Y en escritorio mediante anclajes `sm:`.
- **Modo Oscuro Neón / Translúcido (Glassmorphic):** Rediseño de las variables CSS de swatches en `.dark` usando `rgba(..., 0.15)` y tonos de texto vibrantes (400) para asegurar fidelidad exacta a nombres de colores (Rosa, Fucsia, etc.).
